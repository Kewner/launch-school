...
function minilang(str) {
  const correctTokens = ['PUSH', 'ADD', 'SUB', 'MULT',
                         'DIV', 'MOD', 'POP', 'PRINT'];
  ...
  for (let idx = 0; idx < tokens.length; idx++) {
    ...
    if (Number(token)) {
      ...
      } else if (token !== 'PUSH' && token !== 'PRINT' && stack.length === 0) {
...
// LS solution uses forEach and doesn't first check if the token is a number;
// the last check of the switch statement does this with the 'default' case.
// ==========================================================================

function minilang2(program) {
  const cmnds = ['PUSH', 'PRINT', 'ADD', 'SUB', 'MULT', 'DIV', 'MOD', 'POP'];
  let stack = [];
  let register = 0;

  program.split(" ").forEach(token => {

    if (cmnds.slice(2).includes(token) && stack.length === 0) {
      console.error("Stack is empty, operation impossible!");
    }
