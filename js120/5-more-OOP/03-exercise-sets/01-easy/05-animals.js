...
/* When calling `cat.introduce()`:
1. JS doesn't find it on the `cat` instance object.
2. JS looks up in the prototype chain, finds it on `Cat.prototype`, and calls it.
3. `super.introduce()` looks for `introduce()` on `Cat.prototype`s parent, which is `Pet.prototype`.
4. It calls this method, which returns the first part of the end result string.
